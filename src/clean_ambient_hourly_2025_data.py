...
def filter_data_include_distance_away(hexDF: pd.DataFrame, transitDF: pd.DataFrame, maxDistance: int):
    ...
    for index, row in hexDF.iterrows():
        ...
        hexNotFoundInHash = hashmap.get(rowID) is None

        if hexNotFoundInHash:
            #print(rowID + " is not in the hash")
            for stationIndex, stationRow in transitDF.iterrows():
                features = stationRow['features']
            ...
            #print(rowID + " IS IN THE HASH")
            hexWithinRadius = hashmap.get(rowID) is True

        if hexWithinRadius:
            rows_list.append(row)

        if (rowsUpdated%1000 == 0):
            print("Updated " + str(rowsUpdated) + " rows")
...
def main():
    ...
    if SAVE_DATA:
        ...
        save_data(cleanedDF, maxDistance)
